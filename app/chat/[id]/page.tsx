...
export default function TicketChat({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [ticketData, setTicketData] = useState<any>(null); // Trzymamy dane ticketu w state
    ...
    const fetchTicketAndHistory = async () => {
      ...
      if (comments && comments.length > 0) {
        ...
        if (ticket.status === "AI_OPERATED" || ticket.status === "WAITING_FOR_USER") {
          ...
          await callAI(prompt, true, []); // Pusta historia na start
        } else {
          setLoading(false);
        }
      }
  ...
  // KLUCZOWA ZMIANA: Przekazujemy tablicę aktualnych wiadomości (history) do funkcji
  const callAI = async (messageText: string, isInitial = false, history: Message[] = []) => {
    ...
    // Sklejamy całą dotychczasową rozmowę z tablicy w jeden tekst
    const chatHistoryText = history.map(m => `${m.role === 'user' ? 'Klient' : 'AI'}: ${m.text}`).join('\n');
    
    // Uświadamiamy AI o tym co zrobiliśmy do tej pory
    const payloadPrompt = isInitial 
      ? messageText 
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        // Wysyłamy do backendu nasz gruby, "wypchany pamięcią" prompt
        body: JSON.stringify({ message: payloadPrompt, ticketId }),
      });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (ticketStatus === "ESCALATED" || ticketStatus === "IN_PROGRESS") {
      ...
      // Ważne: Przekazujemy 'messages' aby funkcja mogła skleić kontekst
      callAI(text, false, messages); 
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
        {isHumanMode && (
          <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-2xl text-sm font-medium text-center shadow-sm">
        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] md:max-w-[70%] p-5 rounded-[26px] shadow-sm ${msg.role === "user" ? "bg-blue-600 text-white rounded-tr-none" : "bg-white border border-slate-200 rounded-tl-none text-slate-800"}`}>
              ...
              {msg.role === "user" ? <p className="font-medium">{msg.text}</p> : (
                <div className="prose prose-sm max-w-none prose-strong:text-blue-600 text-slate-700">
        ...
        {loading && <div className="text-slate-400 font-bold animate-pulse pl-4">Asystent pisze...</div>}
      </main>
        ...
        <div className="max-w-4xl mx-auto">
          {isClosed ? (
             <div className="text-center py-4 bg-slate-50 rounded-[30px] border-2 border-dashed border-slate-200">
               ...
               <button onClick={() => router.push("/")} className="bg-slate-900 text-white px-10 py-3 rounded-xl font-bold hover:bg-slate-800">Wróć na stronę główną</button>
             </div>
          ) : (
            <form onSubmit={handleSubmit} className="relative flex items-center">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} disabled={loading} placeholder={isHumanMode ? "Napisz do technika..." : "Napisz do asystenta AI..."} className="w-full bg-slate-50 border-2 border-slate-100 rounded-[22px] py-5 pl-7 pr-20 outline-none focus:border-blue-500 font-bold text-slate-900 placeholder:text-slate-400 transition-all disabled:opacity-50" />
